...
def hungarian_method_min(cost_matrix):
    """
    Implementasi lengkap metode Hungarian untuk penugasan optimal minimum.
    """
    steps = []
    matrix = np.array(cost_matrix, dtype=float)
    n = matrix.shape[0]

    # Tabel 1: Tanpa transformasi untuk pencarian minimum
    steps.append({
        'title': 'Tabel 1: Matriks Awal',
        'matrix': matrix.copy()
    })

    # Tabel 2: Pengurangan Kolom
    col_mins = np.min(matrix, axis=0)
    col_reduced_matrix = matrix - col_mins
    steps.append({
...
def main():
    st.title("🔢 Metode Hungarian (Penugasan Optimal Minimum)")
    
    # Input matriks
    st.subheader("Masukkan Matriks Biaya")
    ...
    if st.button("Hitung Penugasan Optimal Minimum"):
        steps = hungarian_method_min(matrix)
        ...
        for step in steps:
            ...
            if step['title'] == 'Tabel 6: Penugasan Optimal':
                ...
                st.subheader("Hasil Penugasan Optimal Minimum")
                assignment_text = []
                ...
                st.write("Total Biaya:", total_cost)
